...
{
    [PollingEvent("On user created", "Triggered when a new user is added"), MultipleEvents]
    public async Task<PollingEventResponse<PollingMemory, List<UserDto>>> OnUsersCreated(
    ...
    private static PollingEventResponse<PollingMemory, List<UserDto>> NoUsers()
        => new()
        {
            FlyBird = false,
            Result = null,
